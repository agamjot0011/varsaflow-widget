(function () {
  ...
  function renderBotResponse(rawData) {
    ...
    // Execute webhook actions (scroll, url)
    if (data.action) {
      switch (String(data.action.type || '').toLowerCase()) {
        case 'scroll':
          var target = document.querySelector(data.action.target);
          if (target) {
        ...
        case 'url':
          window.location.assign(data.action.target);
          break;
      }
    }
